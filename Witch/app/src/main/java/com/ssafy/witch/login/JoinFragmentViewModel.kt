...
class JoinFragmentViewModel(application: Application): AndroidViewModel(application) {
    ...
    private val joinService = RetrofitUtil.joinService

    //이메일 중복 확인
    fun checkEmailUnique(email: String, onResult: (Boolean, String?) -> Unit) {
        executeRequest({ joinService.checkEmailUnique(email) }, onResult)
    }

    //닉네임 중복 확인
    fun checkNicknameUnique(nickname: String, onResult: (Boolean, String?) -> Unit) {
        executeRequest({ joinService.checkNicknameUnique(nickname) }, onResult)
    }

    //이메일 인증 코드 요청
    fun requestEmailVerification(email: String, onResult: (Boolean, String?) -> Unit) {
        executeRequest({ joinService.requestEmailVerification(mapOf("email" to email)) }, onResult)
    }

    //이메일 인증 코드 확인
    fun confirmEmailVerification(email: String, code: String, onResult: (Boolean, String?) -> Unit) {
        executeRequest({ joinService.confirmEmailVerification(mapOf("email" to email, "emailVerificationCode" to code)) }, onResult)
    }

    //회원가입 요청
    fun registerUser(request: Join, onResult: (Boolean, String?) -> Unit) {
        executeRequest({ joinService.registerUser(request) }, onResult)
    }
